...
public class AufgabeTableAPI {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Table tableA = teile.filter("Gewicht >= 50")
                            .join(lieferungen.as("LNr, lTNr, PNr, Menge"))
                            .where("TNr === lTNr")
                            .select("PNr.count.distinct as cnt");
        DataSet<Long> resultA = tEnv.toDataSet(tableA, Long.class);
        resultA.print();
        //////////
        ...
        Table tableB = lieferanten.join(lieferungen.as("L, TNr, PNr, Menge"))
                                  .where("LNr === L")
                                  .groupBy("LName")
                                  .select("LName, Menge.sum as Summe")
                                  .orderBy("LName, Summe.asc");
        DataSet<String> resultB = tEnv.toDataSet(tableB.select("LName"), String.class);
        resultB.print();
        DataSet<Long> resultSumme = tEnv.toDataSet(tableB.select("Summe"), Long.class);
        resultSumme.print();
        //////////
        ...
        Table tableC = lieferungen.groupBy("LNr")
                .select("LNr, LNr.count as Anzahl")
                .join(lieferanten.as("L, LName, Ort"))
                .where("LNr === L")
                .filter("Anzahl < 8")
                .select("Ort");
        DataSet<String> resultC = tEnv.toDataSet(tableC, String.class);
        resultC.print();
        //////////
        ...
        Table anzahlVonSchwarzenTeilen = teile.filter("Farbe === 'Schwarz'")
                                              ...
                                              .groupBy("PNr")
                                              .select("PNr, Menge.sum as SchwarzeTeileSumme");
        Table tableD = anzahlVonLieferungen.join(anzahlVonSchwarzenTeilen.as("P, SchwarzeTeileSumme"))
                                           .where("PNr === P")
                                           .filter("(SchwarzeTeileSumme / AnzahlVonLieferungen) > 60")
                                           .select("PNr");
        DataSet<Long> resultD = tEnv.toDataSet(tableD, Long.class);
        resultD.print();
        //////////
